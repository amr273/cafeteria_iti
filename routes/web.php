...
Route::middleware([
    ...
    Route::middleware('role:admin')->prefix('admin')->group(function () {
        ...
        Route::put('/beverages/{beverage}', [AdminWebController::class, 'updateBeverage'])->name('admin.beverages.update');
        ...
        Route::patch('/users/{user}/role', [AdminWebController::class, 'updateUserRole'])->name('admin.users.update-role');
        Route::patch('/orders/{order}/status', [AdminWebController::class, 'updateOrderStatus'])->name('admin.orders.update-status');
        Route::delete('/orders/{order}', [AdminWebController::class, 'destroyOrder'])->name('admin.orders.destroy');

        // إدارة الأقسام
        Route::post('/categories', [AdminWebController::class, 'storeCategory'])->name('admin.categories.store');

        // الشات بوت الإداري
        Route::post('/chatbot/send', [ChatbotController::class, 'handle'])->name('admin.chatbot.send');
    });
});
