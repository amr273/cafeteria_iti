<x-app-layout>
  ...
  <div class="dashboard-shell" style="padding: 32px 0; min-height: 100vh; font-family: system-ui, -apple-system, sans-serif;">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" style="display: flex; flex-direction: column; gap: 28px;">
      ...
      <div class="dashboard-panel" style="padding: 24px; background: linear-gradient(135deg, rgba(99,102,241,0.08), rgba(14,165,233,0.06));">
        <div style="display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 18px; flex-wrap: wrap;">
          <div>
            <h3 style="margin: 0; font-size: 22px; font-weight: 900; color: #0f172a;">💬 الشات بوت الذكي</h3>
            <p style="margin: 6px 0 0; color: #475569; font-size: 14px; font-weight: 600;">مساعد إداري حقيقي يستجيب بناءً على بيانات النظام الحالية</p>
          </div>
          <span style="padding: 8px 14px; border-radius: 999px; background: rgba(79, 70, 229, 0.1); color: #4338ca; font-size: 12px; font-weight: 800;">admin.chatbot.send</span>
        </div>

        <div id="adminChatBox" style="background: rgba(255,255,255,0.85); border: 1px solid rgba(148,163,184,0.18); border-radius: 16px; min-height: 220px; max-height: 300px; overflow-y: auto; padding: 18px; display: flex; flex-direction: column; gap: 12px;">
          <div style="max-width: 85%; align-self: flex-start; background: white; border: 1px solid #e2e8f0; border-radius: 16px 16px 16px 4px; padding: 12px 14px; color: #1f2937; font-weight: 600; box-shadow: 0 8px 18px -18px rgba(15, 23, 42, 0.35);">
            مرحباً بك 👋 أستطيع مساعدتك في متابعة الطلبات، المخزون، والإحصائيات العامة للنظام.
          </div>
        </div>

        <form id="adminChatForm" style="display: flex; gap: 12px; margin-top: 18px; flex-wrap: wrap;">
          <input type="text" id="adminChatInput" placeholder="اكتب سؤالك للمساعد الإداري..." required style="flex: 1; min-width: 220px; border: 1.5px solid #dbe3f0; border-radius: 12px; padding: 12px 16px; font-size: 14px; color: #0f172a; background: #ffffff;">
          <button type="submit" style="background: linear-gradient(135deg, #4f46e5, #4338ca); color: white; border: none; border-radius: 12px; padding: 12px 18px; font-weight: 800; cursor: pointer;">
            إرسال
          </button>
        </form>
      </div>
  ...
  <script>
    ...
    if (adminChatForm && adminChatInput && adminChatBox) {
      adminChatForm.addEventListener('submit', function(event) {
        ...
        appendAdminMessage('أنت', message, 'user');
        ...
        fetch('{{ route('admin.chatbot.send') }}', {
          ...
          .then(data => {
            const reply = data && data.reply ? data.reply : 'أستطيع مساعدتك في متابعة النظام.';
            appendAdminMessage('المساعد', reply, 'bot');
          })
          .catch(() => {
            appendAdminMessage('المساعد', 'حدث خطأ في الاتصال بالذكاء الاصطناعي، حاول مرة أخرى.', 'bot');
          });
      });
    }

    function appendAdminMessage(sender, text, type) {
      const line = document.createElement('div');
      const baseStyle = 'max-width: 85%; padding: 12px 14px; border-radius: 16px; font-weight: 600; line-height: 1.6; white-space: pre-wrap;';

      if (type === 'user') {
        line.style.cssText = baseStyle + ' align-self: flex-end; background: linear-gradient(135deg, #4f46e5, #4338ca); color: white; border-radius: 16px 16px 4px 16px;';
      } else {
        line.style.cssText = baseStyle + ' align-self: flex-start; background: white; border: 1px solid #e2e8f0; border-radius: 16px 16px 16px 4px; color: #1f2937;';
      }

      line.textContent = sender + ': ' + text;
      adminChatBox.appendChild(line);
